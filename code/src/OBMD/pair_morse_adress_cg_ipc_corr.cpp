...
#include "pair_morse_adress_cg_ipc_corr.h"
...
PairMorseAdResSCGIPCCorr::PairMorseAdResSCGIPCCorr(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
  ...
  memory->create(massproc_tmp, nmolecules, "morse/adress/cg/ipc/corr:massproc_tmp");
  memory->create(masstotal_tmp, nmolecules, "morse/adress/cg/ipc/corr:masstotal_tmp");
  memory->create(mol_f_adr, nmolecules, 3, "morse/adress/cg/ipc/corr:mol_f_adr"); // vector
  memory->create(mol_f_all_adr, nmolecules, 3, "morse/adress/cg/ipc/corr:mol_f_all_adr"); // vector
...
PairMorseAdResSCGIPCCorr::~PairMorseAdResSCGIPCCorr()
{
  if (copymode) return;
...
void PairMorseAdResSCGIPCCorr::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        s_factor = 1.0 - 4.0 * pow(sqrt(ilambda*jlambda) - 0.5,2.0);
...
void PairMorseAdResSCGIPCCorr::allocate()
{
  allocated = 1;
...
void PairMorseAdResSCGIPCCorr::settings(int narg, char **arg)
{
...
void PairMorseAdResSCGIPCCorr::coeff(int narg, char **arg)
{
...
double PairMorseAdResSCGIPCCorr::init_one(int i, int j)
{
...
void PairMorseAdResSCGIPCCorr::write_restart(FILE *fp)
{
...
void PairMorseAdResSCGIPCCorr::read_restart(FILE *fp)
{
...
void PairMorseAdResSCGIPCCorr::write_restart_settings(FILE *fp)
{
...
void PairMorseAdResSCGIPCCorr::read_restart_settings(FILE *fp)
{
  /* if (comm->me == 0) {
...
void PairMorseAdResSCGIPCCorr::write_data(FILE *fp)
{
...
void PairMorseAdResSCGIPCCorr::write_data_all(FILE *fp)
{
  /* for (int i = 1; i <= atom->ntypes; i++)
...
double PairMorseAdResSCGIPCCorr::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
...
void *PairMorseAdResSCGIPCCorr::extract(const char *str, int &dim)
{
  dim = 2;
...
int PairMorseAdResSCGIPCCorr::find_mols(tagint &idlo, tagint &idhi) {
