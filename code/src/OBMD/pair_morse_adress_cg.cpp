...
#include "pair_morse_adress_cg.h"
...
PairMorseAdResSCG::PairMorseAdResSCG(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
  ...
  memory->create(massproc_tmp, nmolecules, "morse/adress/cg:massproc_tmp");
  memory->create(masstotal_tmp, nmolecules, "morse/adress/cg:masstotal_tmp");
  memory->create(mol_f_adr, nmolecules, 3, "morse/adress/cg:mol_f_adr"); // vector
  memory->create(mol_f_all_adr, nmolecules, 3, "morse/adress/cg:mol_f_all_adr"); // vector
...
PairMorseAdResSCG::~PairMorseAdResSCG()
{
  if (copymode) return;
...
void PairMorseAdResSCG::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        mol_f_adr[imol][0] +=  lambda_factor * delx * fpair; // delx * fpair;
        mol_f_adr[imol][1] +=  lambda_factor * dely * fpair; // dely * fpair;
        mol_f_adr[imol][2] +=  lambda_factor * delz * fpair; // delz * fpair;
        // std::cout<<"morse/adress/cg/test: after: mol_f_adr[imol][0]: "<<mol_f_adr[imol][0]<<"\n";
        // std::cout<<"ENDING CG HERE"<<"\n";
        if (newton_pair || j < nlocal) {
          mol_f_adr[jmol][0] -=  lambda_factor * delx * fpair; // delx * fpair;
          mol_f_adr[jmol][1] -=  lambda_factor * dely * fpair; // dely * fpair;
          mol_f_adr[jmol][2] -=  lambda_factor * delz * fpair; // delz * fpair;
        }
...
void PairMorseAdResSCG::allocate()
{
  allocated = 1;
...
void PairMorseAdResSCG::settings(int narg, char **arg)
{
...
void PairMorseAdResSCG::coeff(int narg, char **arg)
{
...
double PairMorseAdResSCG::init_one(int i, int j)
{
...
void PairMorseAdResSCG::write_restart(FILE *fp)
{
...
void PairMorseAdResSCG::read_restart(FILE *fp)
{
...
void PairMorseAdResSCG::write_restart_settings(FILE *fp)
{
...
void PairMorseAdResSCG::read_restart_settings(FILE *fp)
{
  /* if (comm->me == 0) {
...
void PairMorseAdResSCG::write_data(FILE *fp)
{
...
void PairMorseAdResSCG::write_data_all(FILE *fp)
{
  /* for (int i = 1; i <= atom->ntypes; i++)
...
double PairMorseAdResSCG::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
...
void *PairMorseAdResSCG::extract(const char *str, int &dim)
{
  dim = 2;
...
int PairMorseAdResSCG::find_mols(tagint &idlo, tagint &idhi) {
