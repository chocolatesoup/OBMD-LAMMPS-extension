...
FixStyle(adress/obmd,FixAdResSObmd)

#else

#ifndef LMP_FIX_ADRESS_OBMD_H
#define LMP_FIX_ADRESS_OBMD_H
...
class FixAdResSObmd : public Fix {
 public:
  FixAdResSObmd(class LAMMPS *, int, char **);
  ~FixAdResSObmd();
  int setmask();
  void init();
  void pre_exchange();
 ...
 private:
  ...
  void find_maxid();
  // to distribute external force on com's
  tagint idlo, idhi;
  int nmolecules;
  ...
  double g_par_global(Region*, int, double*);
  double g_perp_global(Region*, int);  
  double g_par_local(double, double, int); // PPapez removed region
  double energy(int, int, double *, double *);

  // PP 
  // double energy_adress_obmd(double, int, double *, double *); // charge qi, type, coords, fusher
  double energy_cg_adress_obmd(Region *, Region *, int, double *, double *);
  // double energy_cg_adress_obmd(Region *, Region *, double *, double *);
