...
void Molecule::read(int flag)
{
  ...
  while (!keyword.empty()) {
    if (keyword == "Coords") {
      ...
      dbodyflag = 1;
      body(flag, 1, line);
    } else if (keyword == "Representative atoms") { // PP ... read rep_atom info if provided
      repatomflag = 1;
      if (flag)
        repatoms(line);
      else
...
void Molecule::types(char *line)
{
  ...
  for (int i = 0; i < natoms; i++) {
    ...
    std::cout<<"types: iatom: "<<iatom<<"\n";
    std::cout<<"types: natoms: "<<natoms<<"\n";
    ...
    switch (utils::is_type(typestr)) {
      case 0: {    // numeric
        ...
        type[iatom] += toffset;
        std::cout<<"types: iatom: "<<iatom<<" type[iatom]: "<<type[iatom]<<"\n";
        break;
      }
      case 1: {    // type label
        if (!atom->labelmapflag)
...
void Molecule::repatoms(char *line) { // PP
  ...
  for (int i = 0; i < natoms; i++) {
    ...
    // check # of atoms
    int iatom = utils::inumeric(FLERR, values[0], false, lmp);
    std::cout<<"molecule (START)"<<"\n";
    std::cout<<"repatoms: iatom: "<<iatom<<"\n";
    std::cout<<"repatoms: natoms: "<<natoms<<"\n";
    if (iatom < 1 || iatom > natoms)
    ...
    std::cout<<"repatoms: iatom: "<<iatom<<" rep_atom[iatom]: "<<rep_atom[iatom]<<"\n";
    std::cout<<"molecule (END)"<<"\n";
  }
}
...
void Molecule::check_attributes()
{
  ...
  if (repatomflag && !atom->repatom_flag) mismatch = 1; // PPapez COMMENT: not attribute if atomistic/obmd ... for adress/obmd check it manually, for instance, in try_inserting() 
...
void Molecule::allocate()
{
  ...
  if (repatomflag) memory->create(rep_atom, natoms, "molecule:rep_atom"); // PP
