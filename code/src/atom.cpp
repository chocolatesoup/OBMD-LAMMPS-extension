...
#include "atom.h"
#include "atom_vec.h"
#include "OBMD/atom_vec_adress.h"
...
Atom::Atom(LAMMPS *_lmp) : Pointers(_lmp), atom_style(nullptr), avec(nullptr), avec_map(nullptr)
{
  ...
  vcms_mol = nullptr; // PPapez COMMENT: added
...
void Atom::peratom_create()
{
  ...
  add_peratom("lambdaF",&lambdaF,DOUBLE,0);
  add_peratom("cms_mol",&cms_mol,DOUBLE,3);
  add_peratom("rep_atom",&rep_atom,INT,0);

  add_peratom("vcms_mol", &vcms_mol, DOUBLE, 3);
...
void Atom::set_atomflag_defaults()
{
  ...
  lambdaF_flag = cms_mol_flag = 0; //OBMD PP: deleted rep_atom_flag
  vcms_mol_flag = 0;
...
{ 
  // std::cout<<"in add_molecule_atom"<<"\n";
  // std::cout<<"iatom: "<<iatom<<" ilocal: "<<ilocal<<"\n";

  // adress/obmd
  if (onemol->repatomflag && repatom_flag) {
    // std::cout<<"in if(repatomflag)"<<"\n";
    rep_atom[ilocal] = onemol->rep_atom[iatom];
    // std::cout<<"rep_atom[ilocal]: "<<rep_atom[ilocal]<<"\n";
  }
...
void *Atom::extract(const char *name)
{
  ...
  if (strcmp(name,"lambdaF") == 0) return (void *) lambdaF;
  if (strcmp(name,"cms_mol") == 0) return (void *) cms_mol;
  if (strcmp(name,"rep_atom") == 0) return (void *) rep_atom;

  if (strcmp(name, "vcms_mol") == 0) return (void *) vcms_mol; 
